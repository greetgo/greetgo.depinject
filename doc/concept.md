...
а программист бы занимался только решением конкретной задачи. Программисту нужно сказать только то, что ему необходимо в
том или ином месте, и "какая-то волшебная штука" ему это дает, притом сразу.
...
depinject создаёт инстанции бинов только по мере необходимости, то можно в бин-контейнер ложить сотни тысяч бинов и
...
> Инстанции бин-классов создаются в бин-контейнере по мере необходимости. 
...
Аннотации `@Bean` не достаточно, чтобы можно было создавать бин. Бин-класс необходимо ещё подключить к бин-контейнеру.

> Подключение бин-классов к бин-контейнерам происходит с помощью бин-конфигов.

### Бин конфиги

Бин-конфиг - это класс без методов и полей, ничто не наследующий и ничего не расширяющий. Бин конфиг должен быть
...
проекта. Можно создать несколько бин-контейнеров, и каждый бин-контейнер содержит свой набор бинов. Какие-то бины
подключены к одному бин-контейнеру, а какие-то к другому, а какие-то бины могут быть подключены
к нескольким бин-контейнерам.
...
> Разработчик специально сделал так, чтобы в этом случае происходила ошибка. Чтобы не было проблем с возвращением
  `null`; или проблем с непонятностью: а какой бин сюда подключится. А с ошибкой всё понятно: конфигурируй так,
...
Следует понимать, что синглтон в depinject-е содержит одну инстанцию в рамках инстанции бин-контейнера. Если создать
ещё одну инстанцию бин-контейнера, то в ней будут создаваться новые инстанции бинов-синглтонов. Поэтому depinject-овский
синглтон, не совсем синглтон в классическом понимании. Если бин-контейнер сделать классическим синглтоном, то все
бины-синглтоны этого контейнера станут классическими.

### Бин-контейнер

Бин-контейнер представляет из себя интерфейс, который должен расширять интерфейс `BeanContainer`. В интерфейсе
`BeanContainer` нет ни каких методов - он служит лишь как индикатор. Сам же интерфейс бин-контейнера должен содержать
методы без параметров. Имена методов значения не имеют. Имеет значение тип возвращаемого значения.

> Тип возвращаемого значения бин-контейнера должен однозначно определать какой-то один конкретный бин.
...
Бины к бин-контейнеру необходимо подключать с помощью аннотации `@Include`.

Бин-контейнер реализуется автоматически с помощью кодогенерации. Есть метод `DepinjectUtil.implementBeanContainers`
из библиотеки `greetgo.depinject.gen`, который сканирует указанный пакет на наличие интерфейсов бин-контейнера
