### Ссылки
 ...
 - [Спецификация]
   ...
   - [Подключение бинов к бин-контейнерам](#include-beans-to-bean-containers)
   ...
   - [Бин-контейнеры](#bean-containers)
...
Подключение бина к бин-контейнеру делается в два этапа:
  1. Подключение бина к бин-конфигу,
  2. Подключение бин-конфига к бин-контейнеру напрямую или через другой бин-конфиг.

Подключение бина к бин-конфигу делается с помощью [аннотации `@BeanScanner`](#beanscanner-annotation).

Подключение бин-конфига к бин-контейнеру или другому бин-конфигу делается
...
пакетах. Эта аннотация указывается у бин-конфига (рядом с аннотацией `@BeanConfig`). Также в этой аннотации можно
...
Также путь может начинаться с одной или нескольких крышек (^). Одна крышка означает родительский пакет,
...
аннотации `@Include`. Теперь, если мы подключим к бин-контейнеру `BeanConfigHelloWorld`, то бин-контейнеру будет
...
Бин-контейнер - это интерфейс расширяющий интерфейс `@BeanContainer`. Бин-контейнер должен содержать один или несколько
методов. Методы в бин-контейнере не должны содержать аргументов. Иначе будет ошибка сборки.

Тип возвращаемого значения метода бин-контейнера должен однозначно определять один и только обин бин из того набора
бинов, которые подключены к этому бин-контейнеру. Иначе будет ошибка сборки.

Инстанция бин-контейнера создаётся методом `Depinject.newInstance`. На вход этого метода, передаётся класс
бин-контейнера, а на выходе инстанция этого класса.

Класс, реализующий бин-контейнер, должен создаваться специальным генератором, компилироваться и включаться в class path.
Для генерации реализации бин-контейнеров используется библиотека `greetgo.depinject.gen`, и класс `DepinjectUtil`.

###### Replacers
#### Подмена бинов

Библиотека предоставляет возможность подмены бинов, с помощью подменщиков бинов (бин-подменщиков).
...
оригинальный бин, а вторым аргументом передаётся класс из `BeanGetter`-а. Метод должен вернуть объект, который
`instanceOf returnClass`. Если такой объект создать нельза или не нужно, то можно вернуть `originalBean`.

Для создания бин-подменщика необходимо чтобы бин реализовывал этот интерфейс. У бин-контейнера может быть несколько
бин-подменщиков.

Для одного бина тоже может быть несколько бин-подменщиков. В этом случае он их применяет в порядке величины
приоритетности, указанной аннотацией `@ReplacePriority` в бине-подменщике. Если этой аннотации нет, то считается,
что эта величина приоритетности такого бина-подменщика равна нулю. Для одинаковых величин приоритетности
бины-подменщики выстраиваются в алфавитном порядке по имени класса бина-подменщика.

Бин-подменщики применяются не для всех бинов, а только для тех, которые указанны аннотациями:

  - `@ReplaceInstanceOf` - в этой аннотации указывается интерфейс или класс, который должен расширять или реализовывать
                           бин, чтобы к нему применился бин-подменщик;
  - `@ReplaceWithAnn` - в этой аннотации указывается другая аннотация, которой должен быть помечен бин, чтобы к нему
                        применился данный бин-подменщик.

Эти аннотации указываются у бина-подменщика.
