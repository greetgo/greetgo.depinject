...
и добавить её в дистрибутив. В проекте-примере таким дистрибутивом будет jar-файл c main-классом, который в
...
Теперь чтобы это всё запустить, заходим в папку `depinject.quick_start/` и запускаем команду:
...
контейнеру с помощью аннотации `@Include`.
...
всех бин-контейнеров указанного пакета и всей иерархии его подпакетов и после анализа генерирует исходный код
бин-контейнеров. Этот исходный код помещается в папку
