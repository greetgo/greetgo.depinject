...
//    System.out.println(st);
  ...
  @SuppressWarnings("unused")
  public static class GeneratesNotPublicBeanWithoutConstructor {
    private final BeanGetter<Bean1> bean1;
    private final BeanGetter<Bean2> bean2;
    private BeanGetter<Bean3> bean3;

    public GeneratesNotPublicBeanWithoutConstructor(BeanGetter<Bean2> bean2, BeanGetter<Bean1> bean1) {
      this.bean1 = bean1;
      this.bean2 = bean2;
    }
  }
  ...
  @Test
  public void newBeanCreationWithConstructor_NotPublicBeanWithoutConstructor() {
    Context context = new Context();

    Class<?> beanClass = GeneratesNotPublicBeanWithoutConstructor.class;
    ...
    assertThat(error)
        .describedAs(NotPublicBeanWithoutConstructor.class.getSimpleName() + " MUST be thrown")
        .isNotNull();
    assert error != null;
    assertThat(error.beanClass.getName()).isEqualTo(beanClass.getName());
    assertThat(error.beanGetterField.getName()).isEqualTo("bean3");

    String st = TestUtil.extractStackTraceToStr(error);

//    System.out.println(st);

    assertThat(st).contains("Field bean3 is not public in " + beanClass);
    assertThat(st).contains("Depinject cannot inject into not public field.");

  }

  @SuppressWarnings("unused")
  public static class CheckSkipInjectAnnotation {
    private final BeanGetter<Bean1> bean1;
    private final BeanGetter<Bean2> bean2;
    ...
    public CheckSkipInjectAnnotation(BeanGetter<Bean2> bean2, BeanGetter<Bean1> bean1) {
      this.bean1 = bean1;
      this.bean2 = bean2;
    }
  }
  ...
  @Test
  public void newBeanCreationWithConstructor_CheckSkipInjectAnnotation() {
    Context context = new Context();

    Class<?> beanClass = CheckSkipInjectAnnotation.class;
    ...
    //if error absent then @SkipInject is working good!

  }

  // TODO: 18.10.18 Need add to ...test_beans032... tests about private BeanGetter and constructors with arguments.
}
